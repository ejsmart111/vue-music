    ...
    <div>
        ...
        <div v-else class="begin">
            <div class="child one" :style="{ 'background-image': 'url(' + backgroundImage() + ')'}">
                <font-awesome-icon class="play" @click="playAlbum(playlist.id)" size="2x" icon="play-circle" style="cursor:pointer;color: var(--main);margin-top:250px; margin-left:10px;"/>
            </div>
            <div class="child two">
                <p class="title">{{playlist.name}}</p>
                <p class="artist"><span style="color: var(--main)">Username</span> : {{playlist.user_name}}</p>
                <p class="artist"><span style="color: var(--main)">Join Date</span> : {{playlist.creationdate}}</p><br>
          ...
          <table v-if="!loader">
              <thead>
                  <tr>
                      ...
                      <th class="priorit-4" style="width: 25%">
                          <p>Artist</p>
                      </th>
                      <th class="priority-5">
                          <p>Duration</p>
              ...
              <tbody>
                  <tr :class="{ 'playing' : current && current.id == track.id}" v-for="(track, index) in playlist.tracks" :key="index">
                      <td><p>{{index+1}}</p></td>
      ...
      <p class="artist" v-if="!playlist.tracks && !loader" style="color var(--text-light);text-align:center;font-size:18px;font-weight:bolder">Sorry no Info</p>
      <!-- <pre>{{tracks}}</pre> -->
